...
在同一课程主题下，快速生成适合不同班级、不同基础学生的教案版本。T02 是 T01 已确认教案的增强能力，不作为脱离原教案独立使用的新模块。
...
1. 老师在已有原教案中点击“生成变体版本”，系统先保存当前老师确认稿。
2. 老师从低龄版、基础版、进阶版、演出版中选择一种，并可填写补充调整方向。
3. 系统冻结当前确认稿和原教案标题作为本次生成快照。
4. 系统基于该快照异步生成一个新版本，并自动标出新版本的重难点、易错点、适用对象和调整摘要。
5. 生成完成后，系统在详情页并排展示生成时原稿快照与当前变体，变体侧可继续编辑。
6. 老师保存、导出或从该变体继续生成课堂互动方案。
...
1. 同一课程至少可以分别生成 2 种不同版本，系统固定支持低龄版、基础版、进阶版、演出版四种预设。
2. 不同版本之间应体现难度、时长、组织方式或教学重点差异，并明确适用对象和调整摘要。
3. 每个版本都复用原教案的课程关系，但正文、老师编辑稿和 Markdown 导出互相独立。
4. 第一版只允许从原教案生成一级变体，不能从变体继续派生。
5. 原教案修改后，已有变体的对照快照不变化；原教案删除后，变体和快照仍可查看、编辑和导出。
6. 课堂互动预填必须标明来源是原教案还是具体变体，并带入变体的适用对象和调整摘要。

### 边界说明

第一版采用以下明确边界：

- 每次任务只生成一个变体，不做四版本批量生成。
- 版本类型固定为低龄版、基础版、进阶版、演出版，自定义要求通过补充调整方向填写。
- 只保存“原教案 -> 一级变体”关系，不做多级版本树、分支合并或版本回滚。
- 对照页展示生成时完整原稿快照和当前变体，不做逐字自动差异算法。
- 首版只提供 Markdown 导出，不在 T02 中新增 Word 导出。
...
> 2026-07-02 需求确认结论：
>
> T05 第一版固定为“课堂互动方案文本生成”。系统只生成老师可现场照着执行的文字脚本和互动方案，不开发 Web 小游戏，不生成 2D / 3D 游戏，不做语音合成，不控制灯光、投影或背景素材。
...
| 课程主题 | 客家山歌律动 |
| 年龄段 | 8-12 岁 |
| 当前环节 | 开场、热身、动作学习、分组展示、收尾 |
| 互动目标 | 活跃气氛、纠正节奏、鼓励展示、复习动作 |
| 课堂风格 | 活泼、温柔、正式、舞台排练感 |
| 可用时间 | 3 分钟、5 分钟、8 分钟 |
| 学生人数 | 12 人 |
| 场地与材料限制 | 教室空间有限、无道具、可投屏 |
| 关联教案 | 可选，来自已保存教案 |

### 处理流程

1. 老师可以从教案页面直接生成互动方案，也可以单独进入课堂互动页面生成。
2. 系统自动带入课程主题、年龄段、教学目标、课程风格和注意事项。
3. 老师选择课堂环节、互动目标、可用时间和场地限制。
4. AI 生成课堂互动方案、小游戏规则、老师口令和学生动作说明。
5. 老师编辑后保存为互动方案版本。
6. 上课时老师可直接查看、打印或导出 Markdown。
...
1. 互动方案必须适合对应年龄段和课堂环节。
2. 输出不能过长，要能让老师现场照着执行。
3. 至少包含一个课堂小游戏或互动动作。
4. 必须包含老师口令、学生动作说明和安全提醒。
5. 老师可以编辑、保存、再次查看和导出 Markdown。
6. AI 失败时有明确失败提示，不影响已保存教案。
...
1. 老师打开一份已保存教案。
2. 点击“生成课堂互动方案”。
3. 系统自动带入课程主题、年龄段、教学目标、课程风格和注意事项。
4. 老师补充教学环节、互动目标、可用时间和场地限制。
5. 系统异步生成互动方案。
6. 老师编辑、保存、导出。
...
第一版只做课堂互动方案文本生成。以下内容不进入第一版范围：Web 小游戏、2D / 3D 游戏、学生扫码互动、实时计分、语音合成、方言语音、背景图生成、音乐生成、灯光或投影设备控制。

后续可作为增强方向的内容包括：固定模板 Web 小游戏、课堂抽签与分组计分工具、TTS 朗读口令、背景图或投屏素材生成、与示范材料和动作图解模块联动。
