...
> 面向技术组执行版本  
> 开发周期：2026 年 6 月 17 日 - 2026 年 7 月 20 日  
...
3. 能生成歌舞剧剧本结构、人物设定、唱段适配建议、分角色训练计划、歌舞融合建议。
...
本项目不建议按“一个人只写前端、一个人只写后端”的方式分工。原因是系统规模不大，但每个功能都同时涉及页面、接口、AI 提示词、异步任务和演示数据，如果按前后端切开，容易出现一个人只知道局部、联调压力集中在最后的问题。

建议采用 **业务闭环主责 + 交叉复核** 的方式：每个模块有一个主负责人，但主负责人要把这一条业务线从页面、接口、数据、AI 调用、导出和演示样例整体跑通；另一名成员负责代码审查、体验检查和兜底补位。

| 成员 | Codex 账号条件 | 建议定位 | 主要承担 |
|---|---|---|---|
| 林敬舒 | GPT Plus | 产品流程、教学内容和演示闭环主责 | 需求细化、教学辅助主线、歌舞剧创编主线、提示词初稿、页面体验、演示脚本和文档维护 |
| 李颖昌 | GPT Pro 5x | 技术骨架、复杂 AI 链路和系统联调主责 | 项目骨架、异步任务体系、视频分析链路、动作生成链路、复杂代码生成、测试补全和联调排障 |

具体按业务线分工如下：

| 业务线 | 主负责人 | 协作方式 |
|---|---|---|
| 项目骨架与工程规范 | 李颖昌 | 林敬舒参与目录结构、命名规范、页面菜单和文档约定确认 |
| 教案生成与课堂互动 | 林敬舒 | 李颖昌协助接口结构、JSON 校验、导出和异常处理 |
| 示范视频 / 动作图解制作 | 李颖昌 | 林敬舒负责动作描述、教学可用性判断和示范材料验收 |
| 课后居家练习与纠错 | 李颖昌 | 林敬舒负责标准动作采集方案、老师视角报告内容和学生端体验检查 |
| 歌舞剧创编与分角色训练 | 林敬舒 | 李颖昌协助数据结构、生成任务、保存导出和联调 |
| 排练复盘与最终演示 | 林敬舒 | 李颖昌负责系统稳定性、备用数据、部署和现场技术兜底 |

Codex 使用上，建议按账号额度做轻重任务分配：

| 任务类型 | 建议使用账号 | 原因 |
|---|---|---|
| 大范围代码生成、跨文件重构、复杂报错排查 | GPT Pro 5x | 上下文更长、额度更充足，适合连续多轮联调 |
| 单页面、单接口、提示词模板、文档更新 | GPT Plus | 任务边界清楚，适合小步快跑 |
| 关键模块代码审查 | 两个账号交叉使用 | 主写的人不做唯一审查，避免遗漏业务和工程问题 |
| 演示前全流程检查 | GPT Pro 5x 主跑，GPT Plus 复核 | 用额度更充足的账号做长链路检查，另一个账号按需求文档核对遗漏 |
...
剧本生成作为独立模块，不和教案混在一起。生成后可继续调用唱段适配、分角色训练计划和歌舞融合建议。

### 7.6 唱段适配与词曲改编

M03 的目标不是让系统自动作曲或自动完成专业编曲，而是帮助编导和音乐负责人把“已有歌曲 / 歌词 / 音乐段落”整理成可排练的唱段结构，并给出与剧情相匹配的歌词改写建议。

第一版建议先做 **M03-lite：唱段结构标注和歌词改写建议**。它作为 M04 歌舞融合建议的前置输入，先把“哪里唱、谁来唱、哪几句适合改、哪里留给舞蹈间奏”整理清楚。完整的音频节拍分析、曲谱解析和自动旋律适配放到后续增强。

输入内容：

| 字段 | 示例 | 第一版处理方式 |
|---|---|---|
| 剧本 ID | 客家山歌小剧场 v1 | 必填，系统读取剧情段落、角色和情绪 |
| 关联剧情段落 | 第二幕：孩子们学习客家山歌 | 必填，可从剧本段落中选择 |
| 原曲名称 | 客家山歌类曲目 | 可选，用于说明风格和来源 |
| 原歌词文本 | 用户粘贴歌词 | 推荐输入，AI 主要基于歌词文本改写 |
| 音乐段落表 | 0:00-0:18 前奏；0:18-0:55 主歌 | 推荐输入，由老师或音乐负责人手动整理 |
| 目标表达 | 思念家乡、集体合作、热烈收束 | 必填，用于约束改词方向 |
| 演唱角色 | 主角、奶奶、旁白、全体 | 可从剧本角色中勾选 |
| 改写强度 | 轻微改词 / 明显改编 / 只做结构标注 | 必填，避免 AI 过度改动原歌词 |

输出内容：

- 唱段标题和适用剧情段落。
- 音乐段落结构：前奏、主歌、副歌、间奏、尾声等。
- 歌词改写建议：保留原句、建议替换句和替换理由。
- 演唱分配：独唱、对唱、齐唱、旁白衔接。
- 角色对应关系：每个唱段建议由哪些角色或队伍完成。
- 情绪和表演提示：每段的情绪、动作状态和舞蹈留白。
- 间奏舞蹈位置：哪些时间段或歌词间隙适合群舞、走位或队形变化。
- 与剧情衔接说明：唱段如何承接前后剧情。
- 老师 / 音乐负责人复核提醒：需要人工确认押韵、节拍和版权风险的地方。
...
```json
{
  ...
  "adaptation_goal": "表现孩子们从陌生到喜欢客家山歌的过程",
  "sections": [
    {
      "section_no": "A1",
      "music_position": "0:18-0:55 主歌一",
      "original_lyrics": "用户粘贴的原歌词片段",
      "adapted_lyrics": "建议改写后的歌词片段",
      "singing_mode": "独唱",
      "suggested_roles": ["主角"],
      "emotion": "好奇、轻柔",
      "dance_opportunity": "以站位表演和轻微律动为主",
      "transition_note": "唱完后由旁白引出同伴回应"
    }
  ],
  "dance_interludes": [
    {
      "music_position": "1:25-1:45 间奏",
      "suggestion": "适合安排 8 人半圆队形变化，主角走到舞台中央"
    }
  ],
  "review_notes": [
    "歌词改写需要音乐负责人确认是否贴合原曲节拍。",
    "如涉及已有歌曲，应确认使用授权或改为原创填词。"
  ]
}
```
...
6. 前端用表格和分段编辑器展示结果，支持老师逐句修改歌词、角色和演唱方式。
7. 保存后的唱段适配结果可以被 M04 歌舞融合建议读取，作为“唱段”输入。
8. 支持 Markdown 导出，导出内容包含歌词版本、演唱分配和人工复核提醒。

模型选择：

第一版使用 DeepSeek、通义千问或 OpenAI 兼容的通用文本模型即可，不需要多模态音乐大模型。因为第一版处理的是老师提供的歌词和音乐段落说明，核心任务是文本改写、结构标注和排练建议生成。

后续增强可以分三档推进：

| 增强方向 | 实现方式 | 是否进入第一版 |
|---|---|---|
| 音频时长读取 | FFmpeg / librosa 读取总时长和基础波形信息 | 可选 |
| 简单 BPM / 节拍估计 | librosa 做粗粒度估计，结果仅供参考 | 不作为验收 |
| 自动识别主歌 / 副歌 / 间奏 | 音乐信息检索模型或多模态音频模型 | 暂不做 |
| 曲谱解析和旋律适配 | MusicXML / MIDI 解析 + 专业音乐规则 | 暂不做 |
| 自动作曲 / 编曲 | 专门音乐生成模型 + 人工审核 | 暂不做 |

边界说明：

- 第一版不承诺歌词与原曲旋律完全严丝合缝。
- 第一版不做自动作曲、自动编曲、曲谱解析和专业版权判断。
- 如果上传音频，只作为附件或后续增强素材，不阻塞唱段结构生成。
- 输出必须提醒“歌词、节拍、押韵和版权由音乐负责人最终确认”。
...
截至 2026 年 6 月 17 日，项目尚未进入正式开发，因此开发计划从 6 月 17 日重新起算。计划只作为阶段目标，不按日锁死任务。实际执行时遵循“先跑通主流程，再补体验和兜底”的原则，每个阶段结束前都要留下一个可演示版本。

| 阶段 | 时间范围 | 主要目标 | 阶段交付 |
|---|---|---|---|
| 阶段一：项目启动与骨架 | 6 月 17 日 - 6 月 20 日 | 冻结 MVP 范围，搭建 React 前端、FastAPI API 服务、Python Worker、数据库和对象存储骨架 | 系统能启动，前后端能互通，基础登录、布局和文件上传链路可跑 |
| 阶段二：教学辅助主线 | 6 月 21 日 - 6 月 29 日 | 完成教案生成、课堂互动脚本、示范材料管理、Markdown / Word 导出 | 老师能输入课程条件，生成、编辑、保存并导出教学材料 |
| 阶段三：课后练习分析 | 6 月 30 日 - 7 月 7 日 | 完成学生视频打卡、异步任务状态、基础姿态分析、练习报告生成 | 使用一组标准动作视频和一组学生练习视频跑通“上传 -> 分析 -> 报告 -> 老师复核”流程 |
| 阶段四：歌舞剧创编 | 7 月 8 日 - 7 月 12 日 | 完成剧本结构、人物设定、唱段适配、分角色训练计划、歌舞融合建议 | 编导或老师能生成并编辑一套歌舞剧创编材料 |
| 阶段五：联调与打磨 | 7 月 13 日 - 7 月 17 日 | 完成复盘报告、权限与导出联调、异常兜底、演示数据准备 | 系统进入功能冻结，形成交付候选版本 |
| 阶段六：演示交付 | 7 月 18 日 - 7 月 20 日 | 完成演示脚本、备用数据、现场彩排和阻塞问题修复 | 最终演示材料和可运行系统准备完成 |

### 8.2 阶段内工作方式

每个阶段内部不再细拆到每天，而是按以下方式推进：

1. 每个阶段开始时，只确定本阶段必须跑通的 2-3 条核心流程。
2. 前后端通过 OpenAPI 文档和 mock 数据并行开发，避免互相等待。
3. AI 能力先服务固定演示案例，再逐步扩展输入范围。
4. 每完成一个核心流程，就同步准备示例数据和备用结果。
5. 如果某个 AI 能力效果不稳定，优先保留人工复核、示例结果和降级展示，不影响主流程演示。

### 8.3 阶段验收点

| 时间节点 | 验收标准 |
|---|---|
| 6 月 20 日左右 | 项目骨架能启动，前后端能互通 |
| 6 月 29 日左右 | 教案、互动、示范材料三件事能跑通 |
| 7 月 7 日左右 | 学生视频打卡和练习报告能跑通 |
| 7 月 12 日左右 | 歌舞剧创编和分角色训练能跑通 |
| 7 月 17 日左右 | 系统进入冻结，准备最终演示材料 |
| 7 月 20 日 | 完成交付演示 |
