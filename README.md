...
AI 歌舞剧教学与排演辅助系统。当前仓库已接入课前教案生成、课堂互动方案、歌舞剧剧本创编、唱段适配、歌舞融合、分角色训练计划、排练 / 演出复盘和课后练习提交复核。系统通过 FastAPI 创建 AI 生成任务，Redis 负责排队，Python Worker 调用 DeepSeek 或百炼 Qwen 生成结构化初稿，前端提供可编辑、可保存、可导出的工作台；M05 支持在保留整份计划导出的同时，按角色保存并导出独立 Markdown 训练卡。
